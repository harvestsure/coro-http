...
public:
    ...
    void stream_events(const HttpRequest& request, SseEventCallback callback) {
        auto url_info = parse_url(request.url());
        ...
        if (url_info.is_https) {
            stream_events_https(req_with_cookies, url_info, callback);
        } else {
            stream_events_http(req_with_cookies, url_info, callback);
        }
    }
    
    void stream_events_http(const HttpRequest& request, const UrlInfo& url_info,
    ...
    void stream_events_https(const HttpRequest& request, const UrlInfo& url_info,
                             SseEventCallback callback) {
        rate_limiter_.acquire();
